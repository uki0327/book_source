...
CDateTimeDemoDlg::CDateTimeDemoDlg(CWnd* pParent /*=NULL*/)
	...
	, m_timeLeft(COleDateTime::GetCurrentTime())
	, m_timeRight(COleDateTime::GetCurrentTime())
{
...
{
	CDialog::DoDataExchange(pDX);
	DDX_MonthCalCtrl(pDX, IDC_MONTHCALENDAR1, m_timeLeft);
	DDX_DateTimeCtrl(pDX, IDC_DATETIMEPICKER1, m_timeRight);
}
...
BEGIN_MESSAGE_MAP(CDateTimeDemoDlg, CDialog)
	...
	ON_WM_QUERYDRAGICON()
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_Button_Calc, &CDateTimeDemoDlg::OnBnClickedButtonCalc)
END_MESSAGE_MAP()


// CDateTimeDemoDlg 메시지 처리기
...
void CDateTimeDemoDlg::OnBnClickedButtonCalc()
{
	UpdateData();
	m_timeRight = (m_timeLeft + COleDateTimeSpan(100, 0, 0, 0));
	UpdateData(FALSE);
}
